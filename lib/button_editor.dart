...
class _ButtonEditorState extends State<ButtonEditor> {
  ...
  void _editButton(int index) {
    ...
    final pxCtrl = TextEditingController(text: btn.uiPosX.toInt().toString()); // ←
    final pyCtrl = TextEditingController(text: btn.uiPosY.toInt().toString()); // ←
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    _label('UI Position (Offset)'),
                    Row(
                      children: [
                        Expanded(child: _field(pxCtrl, 'X Offset', num: true)),
                        const SizedBox(width: 12),
                        Expanded(child: _field(pyCtrl, 'Y Offset', num: true)),
                      ],
                    ),
                    // ... [baaki side, swipe, icon, hold code same rahenga]
                    // Side Selection
                    const SizedBox(height: 10),
                    _label('Screen Side'),
                    Row(
                          ...
                          child: GestureDetector(
                            ...
                            child: AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                ...
                                color: side == 'left' ? Colors.white : const Color(0xFF222222),
                                border: Border.all(
                                    color: side == 'left' ? Colors.white : Colors.white24),
                              ),
                              child: Text(
                                'LEFT',
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  color: side == 'left' ? Colors.black : Colors.white54,
                        ...
                        Expanded(
                          child: GestureDetector(
                            ...
                            child: AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                color: side == 'right' ? Colors.white : const Color(0xFF222222),
                                border: Border.all(
                                    color: side == 'right' ? Colors.white : Colors.white24),
                              ),
                              child: Text(
                                'RIGHT',
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  color: side == 'right' ? Colors.black : Colors.white54,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 13,
                                ),
                              ),
                  ...
                  onPressed: () {
                    setState(() {
                      _buttons[index] = CustomButton(
                        id: btn.id,
                        name: nameCtrl.text.toUpperCase().isNotEmpty ? nameCtrl.text.toUpperCase() : 'BTN',
                        icon: icon,
                        isHold: isHold,
                        ...
                        uiPosX: double.tryParse(pxCtrl.text) ?? btn.uiPosX,
                        uiPosY: double.tryParse(pyCtrl.text) ?? btn.uiPosY,
                        swipeDir: swipeDir,
  ...
  Widget _field(TextEditingController ctrl, String label, {bool num = false}) => TextField(
        controller: ctrl,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Button Config', style: TextStyle(color: Colors.white)),
        actions: [
          TextButton(
            onPressed: _save,
      ...
      body: _buttons.isEmpty
          ? const Center(child: Text('No buttons\nTap + to add', textAlign: TextAlign.center, style: TextStyle(color: Colors.white38)))
          : ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return Container(
                  ...
                  child: Row(
                    children: [
                      Icon(iconFromName(b.icon), color: Colors.white70, size: 26),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              ...
                              'Offset:${b.uiPosX.toInt()},${b.uiPosY.toInt()}',
